...
@section('header','Корзина категорий')
@section('content')
    <div class="col-sm-12">
        <section class="panel panel-default">
            <form id="all-post-form" action="{{ route('delete_chose_cat') }}" method="POST">
                {!! csrf_field() !!}
                            ...
                            <td>
                                ...
                                <div>
                                    <ul class="row-property">
                                        ...
                                        <li><a href="#cat_del_{{$cat->cat_id}}" class="cat_del_link del-link">Удалить</a></li>
                                        <li><a href="#restore_{{$cat->cat_id}}" class="restore_link">Востановить</a></li>
                                    </ul>
                                </div>
                            </td>
                            <td>{{ $cat->cat_slug }}</td>
    ...
    @foreach($categories as $cat)
        <form method="POST" id="cat_del_{{$cat->cat_id}}" action="{{ route('delete_cat',['cat_id' => $cat->cat_id]) }}" accept-charset="UTF-8" style="display: none;" >
            <input name="_method" type="hidden" value="DELETE">
            <input name="_token" type="hidden" value="{{ csrf_token() }}">
        </form>
        <form method="POST" id="restore_{{$cat->cat_id}}" action="{{ route('restore_cat',['cat_id' => $cat->cat_id]) }}" accept-charset="UTF-8" style="display: none;" >
            <input name="_token" type="hidden" value="{{ csrf_token() }}">
        </form>
    @endforeach
@endsection